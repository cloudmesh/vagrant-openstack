...
class IPScheduler(driver.Scheduler):
    ...
    def _schedule(self, context, topic, request_spec, filter_properties):
        ...
        if remote_ip.startswith('10.1'):
            ...
            hostname_prefix = 'sridhar'
        #f = open('/home/sridhar/devstack/devstack/filter_log','w')
        #f.write(str(filter_properties))
        #LOG.debug(" = %(filter_properties)s" % locals())
        #LOG.debug("context = %(context)s" % {'context':context.__dict__} )

        LOG.debug("host = %(hosts)s" % {'hosts':hosts})
        LOG.debug("host_name = %(hostname_prefix)s" % {'hostname_prefix':hostname_prefix} )
        #f.flush()
        #f.close()
